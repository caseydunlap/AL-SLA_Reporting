...
jql_query = 'project = "Customer Service Desk" and ("HHAX Platform Region Tag" ~ AL OR "Primary Location" ~ AL OR "HHAX Market" ~ AL)'

with open('C:\\Users\\mdunlap\\Desktop\\Alabama SLA Automation\\Secrets\jira_apikey.txt', 'r') as jira_api_key_temp:
    jira_api_key = jira_api_key_temp.read().replace('\n', '')

with open('C:\\Users\\mdunlap\\Desktop\\Alabama SLA Automation\\Secrets\jira_email.txt', 'r') as jira_login_temp:
...
def SLA_First_Response(temp_filtered_df):
    def check_SLA_response(row):
        ...
        if priority in ['P1- Highest', 'P2- High'] and elapsed_time <= 3:
            return 'Yes'
        elif priority in ['P3- Medium', 'P4- Low'] and elapsed_time <= 72:
            return 'Yes'
        else:
            return 'No'
...
def SLA_Issue_Resolved(sla_df):
    def check_SLA_resolved(row):            
        ...
        if pd.isna(elapsed_time):
            return 'N/A'
        elif priority in ['P1- Highest'] and elapsed_time <= 24:
            return 'Yes'
        elif priority in ['P2- High','P3- Medium', 'P4- Low'] and elapsed_time <= 48:
            return 'Yes'
        else:
            return 'No'
...
#Begin the time series in Feb 2022 
sla_df_final_filtered = sla_df_final[sla_df_final['created_month_year'] >= '2022-02']
...
#Calculate 2022 and 2023 YTD issue values
columns_2022 = [col for col in combined_df.columns if '2022' in str(col)]
combined_df['2022'] = combined_df[columns_2022].sum(axis=1)
columns_2023 = [col for col in combined_df.columns if '2023' in str(col)]
combined_df['YTD'] = combined_df[columns_2023].sum(axis=1)
...
combined_df.iloc[first_response_index, combined_df.columns.get_loc('2022'):] = ''
combined_df.iloc[first_response_index, combined_df.columns.get_loc('YTD'):] = ''
resolved_index = combined_df[combined_df['priority_number'] == 'Resolved'].index[0]
combined_df.iloc[resolved_index, combined_df.columns.get_loc('2022'):] = ''
combined_df.iloc[resolved_index, combined_df.columns.get_loc('YTD'):] = ''
...
final_df_2.to_csv('AL_SLA_Compliance_Monthly_Report.csv')
...
#Load Some Secrets
with open('C:\\Users\\mdunlap\\Desktop\\Alabama SLA Automation\\Secrets\\hha email - from.txt', 'r') as from_email2:
    from_email = from_email2.read().replace('\n', '')
    
email_from = from_email

with open('C:\\Users\\mdunlap\\Desktop\\Alabama SLA Automation\\Secrets\\hha email - to.txt', 'r') as to_email_project:
    to_email = to_email_project.read().strip().split(',')
    
email_members = to_email

with open('C:\\Users\\mdunlap\\Desktop\\Alabama SLA Automation\\Secrets\\google app password.txt', 'r') as app_pass:
...
def send_compliance_report(email_members):
    ...
    with open("AL_SLA_Compliance_Monthly_Report.csv", "rb") as attachment:
        part = MIMEBase("application", "octet-stream")
        part.set_payload(attachment.read())
        encoders.encode_base64(part)
        part.add_header(
            "Content-Disposition",
            f"attachment; filename=AL_SLA_Compliance_Monthly_Report.csv")
        msg.attach(part)
